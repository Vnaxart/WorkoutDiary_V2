...
// Проект раньше не использовал миграции, поэтому у уже созданной MySQL-базы
// могут отсутствовать новые столбцы. Без этого сохранение тренировки падает,
// например с ошибкой Unknown column 'FinishedAt'.
...
static async Task EnsureDatabaseSchemaAsync(IServiceProvider services)
{
    ...
    // Для чистой базы создаём таблицы, а для уже существующей ниже дозаполняем
    // отсутствующие столбцы без ручных SQL-команд от пользователя.
